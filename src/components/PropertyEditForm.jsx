...
export default function PropertyEditForm() {
    ...
    useEffect(() => {
        const fetchProperty = async () => {
            ...
            setPublicIds(property.images);
        };

        fetchProperty();
    }, [id]); // Only run when the component mounts

    // Update the images field in the form state whenever publicIds changes
    useEffect(() => {
        setForm(form => ({ ...form, images: publicIds }));
    }, [publicIds]); // Only run when publicIds changes
